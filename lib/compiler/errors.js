...
const ERROR_CODES = {
  ...
  // Validate phase
  VALIDATE_FILL_IN_AUTO_PARENT:    { severity: 'error',   message: 'FILL child inside AUTO-sized parent collapses to 0px' },
  ...
  VALIDATE_TEXT_NO_STYLE:          { severity: 'error',   message: 'Text node has no text style applied' },
...
class CompilerError extends Error {
  ...
  constructor(code, opts) {
    ...
    const options = opts || {};
    const msg = options.message || (def && def.message) || code;
    super(msg);
    this.name = 'CompilerError';
    this.code = code;
    this.severity = options.severity || (def && def.severity) || 'error';
    this.message = msg;
