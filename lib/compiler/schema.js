...
function missingField(field, nodeName, path) {
  return new CompilerError(
    'PARSE_MISSING_FIELD',
    {
      message: 'Missing required field "' + field + '" on node "' + nodeName + '"',
      node: nodeName,
      path: path + '.' + field
    }
  );
...
function invalidEnum(field, value, allowed, nodeName, path) {
  return new CompilerError(
    'PARSE_MISSING_FIELD',
    {
      message: 'Invalid value "' + value + '" for "' + field + '" on node "' + nodeName +
...
function checkString(field, value, nodeName, path) {
  if (value !== undefined && typeof value !== 'string') {
    return new CompilerError(
      'PARSE_MISSING_FIELD',
      {
        message: 'Field "' + field + '" must be a string on node "' + nodeName + '"',
        node: nodeName,
        path: path + '.' + field
      }
    );
...
function checkNumber(field, value, nodeName, path) {
  if (value !== undefined && typeof value !== 'number') {
    return new CompilerError(
      'PARSE_MISSING_FIELD',
      {
        message: 'Field "' + field + '" must be a number on node "' + nodeName + '"',
        node: nodeName,
        path: path + '.' + field
      }
    );
...
function checkBoolean(field, value, nodeName, path) {
  if (value !== undefined && typeof value !== 'boolean') {
    return new CompilerError(
      'PARSE_MISSING_FIELD',
      {
        message: 'Field "' + field + '" must be a boolean on node "' + nodeName + '"',
        node: nodeName,
        path: path + '.' + field
      }
    );
...
function validateSceneGraph(json) {
  ...
  if (json.version !== "3.0") {
    errors.push(new CompilerError(
      'PARSE_MISSING_FIELD',
      {
        message: 'Scene graph must have version "3.0", got "' + json.version + '"',
        node: null,
        path: 'version'
      }
    ));
