...
/**
 * Collect all variable names referenced by a subtree so build chunks know
 * which globalThis keys they need.
 * @param {object} node
 * @param {Set<string>} out
 */
function collectRefs(node, out) {
  // Walk all string values looking for bridge-exported var names.
  // We conservatively include every import name; the overhead is negligible.
  if (node.children && node.children.length) {
